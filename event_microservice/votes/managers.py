...
class VotedQuerySet(QuerySet):
    def __init__(self, model=None, query=None, using=None, user=None):
        self.user = user
        super(VotedQuerySet, self).__init__(model=model,
                                            query=query,
                                            using=using)
    ...
    def __iter__(self):
        ...
        votes = Vote.objects.filter(content_type=contenttype,
...
class _VotableManager(models.Manager):
    ...
    def down(self, user):
        with transaction.atomic():
            self.through.objects.filter(user=user,
                                        content_object=self.instance).delete()
...
class VotableManager(GenericRelation):
    ...
    def __get__(self, inst, model):
        if inst is not None and inst.pk is None:
